#import "TGCheckersModule.h"
#import "TGCheckersEngine.h"
#import "TGCheckersViewController.h"
#import "../Common/TGGameSaveStore.h"

@implementation TGCheckersModule
- (id)initWithHostContext:(id<TGWorkshopHostContext>)context {
    self = [super init];
    if (self) {
        _hostContext = [context retain];
        _engine = [[TGCheckersEngine alloc] init];
        _saveStore = [[TGGameSaveStore alloc] initWithDataDirectoryURL:[context moduleDataDirectoryURL]
                                                               fileName:@"state.plist"];
        _viewController = [[TGCheckersViewController alloc] initWithEngine:_engine hostContext:context];
    }
    return self;
}
- (NSString *)moduleIdentifier { return @"com.michirose.telegraphica.workshop.checkers"; }
- (NSString *)moduleDisplayName {
    return [_hostContext localizedStringForKey:@"checkers.title" fallback:@"Checkers"];
}
...
- (BOOL)startWithError:(NSError **)error {
    ...
    if (saved && ![_engine restoreFromDictionary:saved]) {
        ...
        [_engine startNewGameWithMode:TGCheckersModeComputer];
