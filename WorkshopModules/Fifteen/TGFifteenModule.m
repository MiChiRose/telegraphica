#import "TGFifteenModule.h"
#import "TGFifteenEngine.h"
#import "TGFifteenViewController.h"
#import "../Common/TGGameSaveStore.h"

static NSString * const TGFifteenModuleIdentifier = @"com.michirose.telegraphica.workshop.fifteen";

@implementation TGFifteenModule

- (id)initWithHostContext:(id<TGWorkshopHostContext>)context {
    self = [super init];
    if (self) {
        _hostContext = [context retain];
        _engine = [[TGFifteenEngine alloc] init];
        _saveStore = [[TGGameSaveStore alloc] initWithDataDirectoryURL:[context moduleDataDirectoryURL]
                                                              fileName:@"state.plist"];
        _viewController = [[TGFifteenViewController alloc] initWithEngine:_engine hostContext:context];
    }
    return self;
}

- (NSString *)moduleIdentifier { return TGFifteenModuleIdentifier; }
