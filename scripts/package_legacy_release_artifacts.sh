...
TDJSON_METADATA_PATH="${TELEGRAPHICA_TDJSON_METADATA_PATH:-}"
TDJSON_MOUNTAIN_LION_METADATA_PATH="${TELEGRAPHICA_TDJSON_MOUNTAIN_LION_METADATA_PATH:-}"
while [ "$#" -gt 0 ]; do
    case "$1" in
        --tdjson)
            shift
...
if [ -z "$TDJSON_METADATA_PATH" ]; then
    TDJSON_METADATA_CANDIDATE="$(dirname "$(dirname "$TDJSON_PATH")")/TDLibBuildMetadata.tsv"
    if [ -f "$TDJSON_METADATA_CANDIDATE" ]; then
        TDJSON_METADATA_PATH="$TDJSON_METADATA_CANDIDATE"
    fi
fi
if [ -z "$TDJSON_MOUNTAIN_LION_METADATA_PATH" ]; then
    TDJSON_MOUNTAIN_LION_METADATA_CANDIDATE="$(dirname "$(dirname "$TDJSON_MOUNTAIN_LION_PATH")")/TDLibBuildMetadata.tsv"
    if [ -f "$TDJSON_MOUNTAIN_LION_METADATA_CANDIDATE" ]; then
        TDJSON_MOUNTAIN_LION_METADATA_PATH="$TDJSON_MOUNTAIN_LION_METADATA_CANDIDATE"
    fi
fi
...
TELEGRAPHICA_TDJSON_METADATA_PATH="$TDJSON_METADATA_PATH" \
TELEGRAPHICA_TDJSON_MOUNTAIN_LION_METADATA_PATH="$TDJSON_MOUNTAIN_LION_METADATA_PATH" \
